...
function Indexer (db, options) {
  if (!(this instanceof Indexer)) return new Indexer(db, options)
  options = options || {}
  var self = this
  ...
  options.properties.forEach(function (key) {
    self.indexes[key] = indexer(db, [key], indexoptions)
  })
}

Indexer.prototype.find = function (index, options) {
  return this.indexes[index].find(options)
}

Indexer.prototype.findOne = function (index, options, cb) {
  return this.indexes[index].findOne(options, cb)
}
