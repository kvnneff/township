...
ActivityTracker.prototype.createFindStream = function (index, options) {
  return this.indexer.find(index, options)
}

ActivityTracker.prototype.filter =
ActivityTracker.prototype.createFilterStream = function (options) {
  return this.db.createReadStream(options).pipe(through.obj(filter(options.query)))
}
